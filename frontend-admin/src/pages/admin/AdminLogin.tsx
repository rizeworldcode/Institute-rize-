...
export default function AdminLogin() {
  ...
  useEffect(() => {
    ...
    const lockExpiry = localStorage.getItem("adminLockout");
    if (lockExpiry) {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        if (newAttempts >= 3) {
          ...
          localStorage.setItem("adminLockout", (Date.now() + lockTime * 1000).toString());
          setLockoutTime(lockTime);
          setError("Too many failed attempts. Account temporarily locked.");
        } else {
          setError(data.message || `Invalid email or password. ${3 - newAttempts} attempts remaining.`);
        }
      }
    } catch (err) {
      ...
      <Reveal>
        <div className="w-full max-w-md relative z-10">
          ...
          <div className="bg-white/80 backdrop-blur-xl border border-white/40 shadow-[0_20px_50px_rgba(0,0,0,0.1)] rounded-3xl p-8 md:p-10 relative overflow-hidden">
            <h1 className="font-display text-3xl font-bold text-neutral-900 mb-2">Admin Portal</h1>
            <p className="text-neutral-500 mb-8">Secure login for staff and administration.</p>
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              <div className="flex justify-end pt-1">
                <Link to="/admin/forget-password" className="text-sm font-semibold text-blue-600 hover:text-blue-700 transition-colors">
                  Forgot Password?
                </Link>
              </div>
